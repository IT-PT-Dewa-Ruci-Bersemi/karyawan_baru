 ...
 *
 * MongoDB Version V.1.0.2
 *
 * V.1.0.2 Changelog
 * - fix fastedit respond.
 */
$.ajaxSetup({
...
var i_formMongo={
    ...
    initButton:function(title,index,id) {
        ...
        if(fields!=null){
            ...
            if(id!==false)temp+='<input type="hidden" name="id" value="'+ i_formMongo._raw_data[id]["id"] +'" />';
            $.each(fields,function(el,val){
                ...
                if(val.type=='hidden'){
                    if(typeof val.source=='undefined')
                        ...
                        temp+='<input type="hidden" name="'+val.name+'" value="'+i_formMongo._raw_data[id][val.source]+'" />';
                    return true;
                    ...
                    $.each(val.field,function(e,v){
                        if(v.type=='timepicker'){_timepicker = true;}
                            ...
                            temp+=i_formMongo.generateButton(autofocus,v,id,_ckeditorFlag);
                            _ckeditor=true;_ckeditorFlag++;return;
                        }
                        else if(v.type=='file'){
                            temp+=i_formMongo.generateButton(autofocus,v,id,_fileFlag);
                            _file=true;_fileFlag++;return;
                        }
                        temp+=i_formMongo.generateButton(autofocus,v,id);
                    });
                    temp+='</div>';
                }else{
                    if(val.type=='timepicker'){_timepicker=true;}
                        ...
                        temp+=i_formMongo.generateButton(autofocus,val,id,_ckeditorFlag);
                        _ckeditor=true;_ckeditorFlag++;return;
                    }
                    else if(val.type=='file'){
                        temp+=i_formMongo.generateButton(autofocus,val,id,_fileFlag);
                        _file=true;_fileFlag++;return;
                    }
                    temp+=i_formMongo.generateButton(autofocus,val,id);
                }
    ...
    generateButton:function(autofocus,val,id,counter){
        var temp='',
            ...
            defValue=id!==false?i_formMongo._raw_data[id][val.name]?i_formMongo._raw_data[id][val.name]:'':'',
        ...
        if(suffix!==false)temp+='<div class="_suffix_notes">'+suffix+'</div>';
        temp+='</div>';
        return temp;
    },
    initGrid:function(param,container){
        ...
        var temp="",a,b,c,i,val,_start_number=0;
            ...
            for(a=0;a<records.length;a++,_start_number++){
                ...
                if(number)temp+='<td>'+_start_number+'</td>';
                for(b=0;b<header.length;b++){
                    ...
                    temp+='<td align="'+header[b].align+'">'+val+'</td>';
                }
                if(menu_action.length || filter!==false){
                    temp+='<td>';
...
$(function () {
    $(document).on('click','.btn-fast-ajax',function () {
        var selector=$(this),
            ...
            headerResult=getObjects(i_formMongo._raw_header, 'data', selector.data('field'));
    ...
    $(document).on('click','.menu-default',function () {
        ...
        $(".modal").remove();
        var temp=i_formMongo.initButton(title,target,id);
    ...
    $(document).on('click', '.menu-order', function () {
        var data=i_formMongo._raw_container.find('tbody').sortable("toArray",{attribute:'data-id'});
